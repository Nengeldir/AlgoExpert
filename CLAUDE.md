...
- **The config freezes on first commit.** `predictor_season` (window, learning rate, tie-break, fill seed, expert pool) is written before the first prediction; `POST /admin/predictor/season` 409s afterwards. The expert pool is frozen against **`window_end`**, so someone who registers mid-season is an expert for the whole run and the days before they registered are filled like any other missing vote. `ln(N)` still feeds the rate and every bound, and N is still fixed across the run — it is merely decided late. **This makes the tick schedule load-bearing:** the first tick freezes the pool for good, so the predictor cron stays disabled until the window has closed and is then enabled once. A tick fired while registrations are open locks in a smaller cohort and silently drops every later joiner (`predictor.test.ts` pins this down).
